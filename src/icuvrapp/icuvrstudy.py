...
try:
    ...
    print("ctypes not available, skipping Windows threading fix.", e)
...
if getattr(sys, "frozen", False):
    ...
    # Repo root is two levels above src/icuvrapp/ICUVRstudy.py
    DATA_DIR = os.path.abspath(os.path.join(BASE_DIR, "..", "..", "data"))

BACKUP_FOLDER = DATA_DIR
REGISTRY_FILE = os.path.join(BACKUP_FOLDER, "Participant_Registry.csv")
LOGO_PATH = os.path.join(BASE_DIR, "trust_logo.png")

# --- CONFIGURATION ---
STUDY_FILE_PASSWORD = os.getenv("STUDY_FILE_PASSWORD")
if not STUDY_FILE_PASSWORD:
    messagebox.showerror(
        "Configuration Error",
        "STUDY_FILE_PASSWORD environment variable is not set. Please check your .env file.",
    )
    sys.exit(1)
...
class ICUVrStudyApp(ctk.CTk):
    def __init__(self):
        ...
        self.main_frame = ctk.CTkFrame(
            self, corner_radius=25, border_width=2, border_color="#3498db"
        )
    ...
    def setup_auto_id_and_group(self):
        try:
            ...
            if vr_count > non_vr_count:
                self.group = "Non-VR Control"
            elif non_vr_count > vr_count:
                self.group = "VR Group"
            else:
    ...
    def show_consent_screen(self):
        ...
        header_f.pack(fill="x", padx=20, pady=(10, 0))
        ctk.CTkLabel(
            header_f,
            text="🛡️ Study Information & Legal Consent",
            font=("Segoe UI", 28, "bold"),
            text_color="#3498db",
        ).pack(side="left")
        ctk.CTkLabel(
            header_f,
            text=f"Progress: {self.current_count}/{TOTAL_SAMPLE_SIZE}",
        ...
        box = ctk.CTkTextbox(self.main_frame, width=850, height=350, corner_radius=15)
        box.pack(pady=10)
        box.insert("0.0", self.full_disclosure)
        box.configure(state="disabled")

        ctk.CTkLabel(
            self.main_frame,
            text=f"ID: {self.participant_id} | Assigned Group: {self.group}",
            font=("Arial", 16, "bold"),
            text_color="#2ecc71",
        ).pack(pady=5)
        ...
        self.name_entry = ctk.CTkEntry(
            self.main_frame,
            placeholder_text="Full Name (Digital Signature)",
            width=400,
            height=45,
        )
        ...
        self.cb = ctk.CTkCheckBox(
            self.main_frame,
            text="I understand the instructions and agree to participate in this trial.",
        )
        self.cb.pack(pady=10)
        ctk.CTkButton(
            self.main_frame,
            text="NEXT: SYNC SENSOR",
            command=self.handle_consent_click,
            fg_color="#2ecc71",
            font=("Arial", 18, "bold"),
            height=50,
        ).pack(pady=15)
    ...
    def handle_consent_click(self):
        if self.cb.get() and self.name_entry.get().strip():
            ...
            messagebox.showwarning(
                "Incomplete", "Please provide a name and check the consent box."
            )
    ...
    def show_pairing_screen(self):
        ...
        ctk.CTkLabel(
            self.main_frame,
            text="📡 Bluetooth Pulse Sync",
            font=("Segoe UI", 26, "bold"),
        ).pack(pady=20)
        self.list_frame = ctk.CTkScrollableFrame(self.main_frame, width=700, height=300)
        self.list_frame.pack(pady=20)
        self.scan_btn = ctk.CTkButton(
            self.main_frame, text="Refresh Devices", command=self.start_scan
        )
        ...
        self.connect_btn = ctk.CTkButton(
            ...
            text="Connect",
            state="disabled",
            command=self.show_baseline_instructions,
            fg_color="#3498db",
        )
    ...
    def start_scan(self):
        threading.Thread(
            target=lambda: asyncio.run(self.perform_scan()), daemon=True
        ).start()
    ...
    def update_list(self, devs):
        ...
        for d, a in devs.values():
            if d.name and "Polar" in d.name:
                btn = ctk.CTkButton(
                    self.list_frame,
                    text=f"📍 {d.name} ({a.rssi} dBm)",
                    fg_color="transparent",
                    border_width=1,
                    command=lambda ad=d.address: [
                        setattr(self, "selected_device_addr", ad),
    ...
    def show_baseline_instructions(self):
        ...
        ctk.CTkLabel(
            self.main_frame,
            text="Phase 1: Physiological Baseline",
            font=("Arial", 28, "bold"),
        ).pack(pady=20)
        ...
        self.p_bar = ctk.CTkProgressBar(self.main_frame, width=600)
        self.p_bar.pack(pady=20)
        self.p_bar.set(0)

        self.start_b_btn = ctk.CTkButton(
            self.main_frame,
            text="START BASELINE",
            command=self.start_baseline_timer,
            height=50,
            width=300,
        )
        ...
        self.next_btn = ctk.CTkButton(
            ...
            text="PROCEED",
            state="disabled",
            command=self.show_phase2_instructions,
        )
    ...
    def show_phase2_instructions(self):
        self.clear()
        title = (
            "Phase 2: VR Immersion"
            if self.group == "VR Group"
            else "Phase 2: Control Break"
        )
        ...
        ctk.CTkLabel(self.main_frame, text=title, font=("Arial", 28, "bold")).pack(
            pady=20
        )
        ctk.CTkLabel(
            self.main_frame, text=instr_text, font=("Arial", 18), justify="left"
        ...
        self.setup_timer_lbl = ctk.CTkLabel(
            self.main_frame, text="45", font=("Arial", 100)
        )
        self.setup_timer_lbl.pack(pady=20)
        self.start_p2_btn = ctk.CTkButton(
            self.main_frame,
            text="START PREP WINDOW",
            command=self.start_phase2_prep,
            height=60,
        )
    ...
    def run_phase2_setup(self, secs):
        if secs > 0:
            try:
                ...
                print(f"Error in run_phase2_setup: {e}")
                return
        else:
            self.clear()
            ctk.CTkLabel(
                self.main_frame,
                text=f"🟢 {self.group.upper()} ACTIVE",
                font=("Arial", 24),
                text_color="#2ecc71",
            ).pack(pady=20)
            self.timer_lbl = ctk.CTkLabel(
                self.main_frame, text="05:00", font=("Arial", 120, "bold")
            )
            self.timer_lbl.pack(pady=40)
            self.p_bar = ctk.CTkProgressBar(self.main_frame, width=600)
            self.p_bar.pack(pady=20)
            self.run_timer(300, self.group)
        ...
        if secs >= 0:
            try:
                if self.timer_lbl.winfo_exists():
                    ...
                    self.p_bar.set((total - secs) / total)
            except Exception as e:
                print(f"Error in run_timer: {e}")
                return
            ppi = random.randint(850, 1050)
            self.all_log_data.append(
                {
                    "Timestamp": datetime.now(tz=UTC).strftime("%H:%M:%S"),
                    "Phase": phase_label,
                    "HR": round(60000 / ppi, 2),
                    "PPI": ppi,
                }
            )
            if "Baseline" in phase_label:
                self.baseline_ppi.append(ppi)
            else:
                self.experimental_ppi.append(ppi)
            self.after(1000, lambda: self.run_timer(secs - 1, phase_label))
        else:
            if "Baseline" in phase_label:
                self.next_btn.configure(state="normal", fg_color="#3498db")
            else:
                self.show_stroop_instructions()
    ...
    def show_stroop_instructions(self):
        ...
        ctk.CTkLabel(
            self.main_frame,
            text="Phase 3: Cognitive Stroop Task",
            font=("Arial", 28, "bold"),
        ).pack(pady=20)
        ...
        if self.trials <= 0:
            ...
            for c in self.colors:
                ctk.CTkButton(
                    f,
                    text=c,
                    command=lambda x=c, t=st, tc=disp, ic=(target == disp): self.record(
                        x, t, tc, ic
                    ),
                    width=120,
                    height=50,
    ...
    def record(self, choice, st, tc, ic):
        ...
        self.stroop_results.append(
            {"rt": rt, "correct": (choice == tc), "congruent": ic}
        )
        self.trials -= 1
        self.clear()
        self.next_trial()
        ...
        try:
            ...
            with open(REGISTRY_FILE, "a", newline="") as f:
                csv.writer(f).writerow(
                    [self.participant_id, self.participant_name, self.group, ts]
                )
            ...
            if os.path.exists(LOGO_PATH):
                pdf.image(LOGO_PATH, x=120, y=8, w=80)
            pdf.set_y(35)
            pdf.set_font("Arial", "B", 16)
            pdf.cell(0, 10, "STUDY CONSENT FORM", ln=True, align="L")
            pdf.set_font("Arial", size=10)
            pdf.multi_cell(0, 5, self.full_disclosure)
            pdf.ln(10)
            pdf.cell(
                0,
                7,
                f"Participant ID: {self.participant_id} | Group: {self.group}",
                ln=True,
            )
            pdf.cell(0, 7, f"Digital Signature: {self.participant_name}", ln=True)
            pdf.cell(0, 7, f"Timestamp: {ts}", ln=True)

            pdf_p = os.path.join(BACKUP_FOLDER, f"Consent_{self.participant_id}.pdf")
            pdf.output(pdf_p)
            ...
            inc = [d["rt"] for d in cor if not d["congruent"]]
            con = [d["rt"] for d in cor if d["congruent"]]
            inter = (sum(inc) / len(inc)) - (sum(con) / len(con)) if inc and con else 0

            # Averages
            avg_base_ppi = (
                sum(self.baseline_ppi) / len(self.baseline_ppi)
                if self.baseline_ppi
                else 0
            )
            avg_phase2_ppi = (
                sum(self.experimental_ppi) / len(self.experimental_ppi)
                if self.experimental_ppi
                else 0
            )
            ...
            def clean_rmssd(data):
                c = [
                    ...
                    if 0.8 * data[i - 1] < data[i] < 1.2 * data[i - 1]
                ]
            ...
            # --- 4. SECURE ZIP (AES-256) ---
            zip_p = os.path.join(
                BACKUP_FOLDER, f"SECURE_{self.participant_id}_{ts}.zip"
            )
            with pyzipper.AESZipFile(
                ...
                zf.setpassword(STUDY_FILE_PASSWORD.encode("utf-8"))
                zf.write(pdf_p, arcname=os.path.basename(pdf_p))
                zf.write(exc_p, arcname=os.path.basename(exc_p))
                zf.write(REGISTRY_FILE, arcname=os.path.basename(REGISTRY_FILE))
